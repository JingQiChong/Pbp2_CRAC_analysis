...
This script relies on "pileup" files in tab-separated text format produced by `pyPileup` script while running the pipeline, put in the directory `~/CRAC_analysis_2022/Pbp2/results/pyPileup_analyses/`. So it must be run after the nf_CRACpipeline, read the repository `README.md` for details.
...
##SML1
## Plot pileup along SML1 transcript as a line
```{r plot_ASH1line,dependson = c("pileup_functions","load_data"),fig.height = 2,fig.width=4}
plot_pileuphits_line(filter(P90_Ab_df, target == "SML1") )
```

## Plot pileup focus on peaks SIM1
```{r plot_SML1focus,dependson = c("pileup_functions","load_data"),fig.height = 4,fig.width=6}
SML1wide_df <- filter(P90_Ab_df, 
                         target == "SML1", 
                         position >= 410, 
                         position <= 500)

P90_Ab_wide_plot <- plot_pileup_one(SML1wide_df, include = "allsubdel") + 
    labs(title = "SML1 , pileup plot") 
    
P90_Ab_wide_plot
##ggsave(plot = SML1wide_plot, filename = "../results/figure_pileup/SML1_CS_plot.pdf",
#       height = 3, width=6, device = "pdf")
```


## Plot pileup focus on SML1 specific coding region

```{r plot_SML1seq,dependson = c("pileup_functions","load_onetable"),fig.height = 3, fig.width=8}
SML1seq_df <- filter(P90_Ab_df, 
                      target == "SML1", 
                      position >=410, 
                      position <=500)
SML1seq_plot <- plot_pileup_one(SML1seq_df, 
                                   include = "allsubdel", 
                                   nucseq = TRUE, nuctxtsize = 2) + 
    labs(title = "SML1 pileup plot")
SML1seq_plot

ggsave(plot = SML1seq_plot, filename = "../results/figure_pileup/SML1_seqplot.png",
...
#pileup_plot_allgenes <- 
 # plot_grid(SML1_widerep_plot,DED1_widerep_plot,NPC2_1_widerep_plot,NPC2_2_widerep_plot ncol = 1) 


pileup_plot_allgenes

#ggsave(plot = pileup_plot_allgenes, 
 #       filename = "../figure_pileup/Composite_CS_CNN_pileupplot.png",
